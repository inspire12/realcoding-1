...
//        return Mono.error(new UnsupportedOperationException("not implemented"));
        return Mono.just(Base62Processor.decode(encoded)).flatMap(decoded -> urlRepository.findById(decoded))
    ...
    public Mono<UrlShortenerResponse> shortenUrl(UrlShortenerRequest urlShortenerRequest) {
        ...
        return urlRepository.save(new URLInformation(url))
                ...
                .map(encoded ->
                        UriComponentsBuilder.newInstance().scheme("http").host(applicationConfig.getHost()).port(applicationConfig.getPort()).path(encoded)
                                .toUriString())
